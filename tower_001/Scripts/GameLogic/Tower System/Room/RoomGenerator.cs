...
public partial class RoomGenerator
{
	...
	private readonly Random _random; // Random number generator for selecting room types and deciding outcomes randomly
	...
	public RoomGenerator(FloorData floorData)
	{
		...
		_random = new Random(); // Initializes the random number generator for random decisions (e.g., selecting room types)
	...
	public bool GenerateRoomsForFloor(int roomCount)
	{
		...
		try
		{
			...
			int guaranteedSpecialRoomIndex = _random.Next(0, roomCount - 1); // Random index excluding the last room
			var specialRoomType = _random.Next(2) == 0 ? RoomType.Reward : RoomType.MiniBoss; // Randomly select reward or miniboss
	...
	//	if (roomCount <= 0 || roomCount > _config.MaxRoomsPerFloor)
	//	{
	//		GD.PrintErr($"Invalid room count: {roomCount}"); // Log error if the room count is invalid
	//		return false; // Return false if the room count is outside the valid range
	//	}

	//	try
	//	{
	//		List<RoomData> rooms = new(); // Initialize a list to store generated room data
	//		var lastRoomPositions = new Dictionary<RoomType, int>(); // Dictionary to track the last position each room type was placed

	//		// Check if this floor is a boss floor
	//		if (_floorData.IsBossFloor)
	//		{
	//			// If it's a boss floor, generate a single boss room and assign it to the floor
	//			_floorData.Rooms = new List<RoomData> { GenerateBossRoom() };
	//			DebugLogger.Log($"Generated boss room for floor {_floorData.FloorNumber}", DebugLogger.LogCategory.Floor);
	//			return true; // Return true to indicate that the rooms were generated successfully
	//		}

	//		// Generate standard rooms for a non-boss floor
	//		for (int i = 0; i < roomCount - 1; i++)
	//		{
	//			// Select a valid room type for the current room position
	//			var roomType = SelectRoomType(i, lastRoomPositions, roomCount);

	//			// Create a new room with the selected type, position, and calculated difficulty
	//			var room = CreateRoom(
	//				roomType,
	//				i,
	//				CalculateRoomDifficulty(i, roomType) // Calculate the room's difficulty based on its position and type
	//			);

	//			rooms.Add(room); // Add the generated room to the list of rooms
	//			lastRoomPositions[roomType] = i; // Update the last position of this room type
	//		}

	//		// The last room will always be a miniboss or reward room
	//		var finalRoomType = _random.Next(2) == 0 ? RoomType.MiniBoss : RoomType.Reward; // Randomly select between MiniBoss and Reward
	//		rooms.Add(CreateRoom( // Create and add the final room to the list
	//			finalRoomType,
	//			roomCount - 1,
	//			CalculateRoomDifficulty(roomCount - 1, finalRoomType) // Calculate the difficulty for the final room
	//		));

	//		_floorData.Rooms = rooms; // Assign the generated rooms to the floor data
	//		return true; // Return true to indicate successful room generation
	//	}
	//	catch (Exception ex)
	//	{
	//		// Catch any errors that occur during room generation and log them
	//		GD.PrintErr($"Error generating rooms: {ex.Message}");
	//		return false; // Return false to indicate that an error occurred during room generation
	//	}
	//}
