...
public partial class RoomManager : BaseManager
{
	...
	private List<RoomData> GenerateNormalFloor(FloorData floor)
	{
		...
		for (int i = 0; i < roomCount - 1; i++)
		{
			...
			var room = CreateRoom(
				...
				i,
				floor.FloorNumber,
				CalculateRoomDifficulty(i, floor.FloorNumber, floor.Difficulty.BaseValue, roomType)
			);
		...
		var finalRoom = CreateRoom(
			...
			roomCount - 1,
			floor.FloorNumber,
			CalculateRoomDifficulty(roomCount - 1, floor.FloorNumber, floor.Difficulty.BaseValue, finalRoomType)
		);
	...
	/// <param name="baseFloorDifficulty">The base difficulty of the floor, which is scaled by room position and type.</param>
	...
	private float CalculateRoomDifficulty(int roomPosition, int floorNumber, float baseFloorDifficulty, RoomType roomType)
	{
		// Start by calculating the base difficulty for the room, factoring in its position on the floor
		float difficulty = baseFloorDifficulty * (1 + (_config.BaseRoomDifficultyIncrement * roomPosition));

		// Apply a difficulty multiplier based on the room type (e.g., Boss rooms are harder than regular rooms)
		difficulty *= roomType switch
		{
			RoomType.Boss => _config.BossRoomDifficultyMultiplier, // Boss rooms have their own multiplier
			RoomType.MiniBoss => _config.MiniBossRoomDifficultyMultiplier, // MiniBoss rooms have a different multiplier
			_ => 1.0f // Default multiplier is 1 (no change to the difficulty)
		};

		// Return the difficulty, ensuring it does not exceed the maximum floor difficulty
		return Math.Min(difficulty, _config.MaxFloorDifficulty);
	}
